...
def create_supervised_evaluator(model, metrics,
    ...
    def _inference(engine, batch):
        id = batch[1]
        id_count = Counter(id)
        id_count = id_count.most_common(len(id_count))
        model.eval()
        with torch.no_grad():
...
def inference(
    ...
    if cfg.TEST.RE_RANKING == 'no':
        ...
        evaluator = create_supervised_evaluator(model, metrics={'r1_mAP': R1_mAP(num_query, cfg, max_rank=50)},
                                                device=device)
